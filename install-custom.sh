...
# Step 15: Write /etc/profile.d/openclaw.sh for system-wide env vars.
# This ensures ALL new shells (login, interactive, SSH) pick up PATH, PNPM_HOME,
# and brew without needing to source ~/.bashrc manually.
# This is the standard Linux mechanism — /etc/profile.d/*.sh files are sourced
# by /etc/profile on every login shell start.
if [[ "$OS" == "linux" ]] && [[ -d /etc/profile.d ]]; then
  info "Writing system-wide environment to /etc/profile.d/openclaw.sh..."
  local_pnpm_home="${PNPM_HOME:-$HOME/.local/share/pnpm}"

  # Build the profile.d script
  PROFILED_CONTENT="#!/bin/sh
# OpenClaw environment — auto-generated by install-custom.sh
export PATH=\"$BIN_DIR:\$PATH\"
export PNPM_HOME=\"$local_pnpm_home\"
export PATH=\"\$PNPM_HOME:\$PATH\"
"

  # Add linuxbrew if installed
  local brew_bin=""
  if [[ -f "/home/linuxbrew/.linuxbrew/bin/brew" ]]; then
  ...
  if [[ -n "$brew_bin" ]]; then
    PROFILED_CONTENT+="eval \"\$($brew_bin shellenv)\"
"
  fi

  run_sudo bash -c "cat > /etc/profile.d/openclaw.sh << 'PROFILED_EOF'
$PROFILED_CONTENT
PROFILED_EOF"
  run_sudo chmod 644 /etc/profile.d/openclaw.sh
  ok "System-wide environment configured (/etc/profile.d/openclaw.sh)"
fi
